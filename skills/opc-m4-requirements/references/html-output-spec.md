...
| M3 方案生成 | Hero｜候选方案（表）｜四维评分与选定（数据卡）｜人的点子（Yes and 引用块）｜产品任务书（表）｜创新点 |
| M4 需求梳理 | Hero｜产品形态（醒目标签）｜八大字段（编号分区：背景/用户/功能/流程/字段/视觉/非功能/验收标准）｜秒哒生成提示词（§3.5，整段纯文本） |
| M5 AI 模拟测试 | Hero｜测试任务｜模拟画像遍历（+/？/−/！）｜**四象限便利贴画布**｜需求挖掘（L1-L4）｜问题分类统计表｜总结（无迭代建议） |
| M6 迭代执行 | Hero｜待决策清单｜修改方案｜发现→修改对照表｜**秒哒可复制指令**（编号文字）｜回归验收清单 |
| M7 路演生成 | Hero｜6 页路演大纲（首页+五段卡片）｜演讲稿底稿｜演示动线（注：M7 另用 dashi-ppt 生成正式 PPT，此 HTML 为大纲版） |
| M8 资产整理 | Hero｜项目基本信息｜时间线｜成果物清单｜关键决策｜方法论/成长｜未来计划｜个人反思（四维度） |
